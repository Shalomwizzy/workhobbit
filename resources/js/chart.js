...
//   var colorPalette = ['#01BFD6', '#5564BE', '#F7A600', '#EDCD24', '#F74F58'];
  ...
  var spark1 = {
    ...
    labels: [...Array(24).keys()].map(n => `2024-05-0${n + 1}`),
    yaxis: {
        min: 0
    },
    xaxis: {
...
var spark2 = {
    ...
    colors: ['#DCE6EC'],
    title: {
        text: updateRegisteredUsers(), // Call the function here to dynamically set the text
        offsetX: 30,
        style: {
            fontSize: '24px',
            cssClass: 'apexcharts-yaxis-title'
...
function updateRegisteredUsers() {
    var totalRegisteredUsers = ''; // Initialize with empty string
    $.ajax({
        url: '/total-registered-users',
        method: 'GET',
        async: false, // Ensure synchronous execution
        success: function(response) {
            if (response && response.totalRegisteredUsers !== undefined) {
                totalRegisteredUsers = response.totalRegisteredUsers.toString(); // Convert to string
            } else {
    ...
    return totalRegisteredUsers; // Return the total registered users
}
...
var spark3 = {
    ...
    labels: [...Array(24).keys()].map(n => `2024-05-0${n + 1}`), // These are just placeholder labels
    xaxis: {
        type: 'datetime',
    },
    yaxis: {
    ...
    subtitle: {
        text: 'Email Subscibers', // This will be updated dynamically
        offsetX: 30,
        style: {
            fontSize: '14px',
            cssClass: 'apexcharts-yaxis-title'
...
// Function to update chart subtitle with total subscribers
function updatetotalSubscribers() {
    var totalSubscribers = '';
    $.ajax({
        url: '/total-subscribers',
        method: 'GET',
        async: false, // Ensure synchronous execution
        success: function(response) {
    ...
    return totalSubscribers; // Return the total subscribers
}
